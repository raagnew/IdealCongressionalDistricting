...
date() # Time
# Read in Kansas census block group data
# Downloaded KS data from https://www2.census.gov/programs-surveys/decennial/2020/data/01-Redistricting_File--PL_94-171/
# Trimmed data in KS_Extract.R script
data <- read.table("c:/Congressional_Districting/KS_Blkgrps.txt",sep=",",header=TRUE)
...
range(a[,2])
k <- 4 # Number of clusters = number of KS congressional districts

# Constrained k-means clustering applied to KS congressional districting
...
write.table(cbind(data,CLUSTER),file="c:/CONGRESSIONAL_DISTRICTING/KS_Blkgroup_Clusters.txt",sep=",",row.names=FALSE,col.names=TRUE)

# Geoplot of KS census block groups and cluster centroids
pdf("c:/CONGRESSIONAL_DISTRICTING/KS_Census_Block_Group_Centroids.pdf")
plot(a[,1],a[,2],col="black",pch=19,cex=.25,asp=1.5,xlab="LONGITUDE",ylab="LATITUDE",
main="KANSAS CENSUS BLOCK GROUPS & CLUSTER CENTROIDS")
points(MEAN_LONGITUDE,MEAN_LATITUDE,col="red",pch=19,cex=.75)
dev.off()

# Geoplot of KS census block group clusters
pdf("c:/CONGRESSIONAL_DISTRICTING/KS_Census_Block_Group_Clusters.pdf")
plot(a[,1],a[,2],asp=1.5,cex=.25,col=rainbow(k)[c],pch=19,xlab="LONGITUDE",ylab="LATITUDE",
main="KANSAS CENSUS BLOCK GROUP CLUSTERS")
dev.off()
date() # Time
