...
date() # Time
# Read in New Hampshire census block group data
# Downloaded NH data from https://www2.census.gov/programs-surveys/decennial/2020/data/01-Redistricting_File--PL_94-171/
# Trimmed data in NH_Extract.R script
data <- read.table("c:/Congressional_Districting/NH_Blkgrps.txt",sep=",",header=TRUE)
...
range(a[,2])
k <- 2 # Number of clusters = number of NH congressional districts

# Constrained k-means clustering applied to NH congressional districting
...
write.table(cbind(data,CLUSTER),file="c:/CONGRESSIONAL_DISTRICTING/NH_Blkgroup_Clusters.txt",sep=",",row.names=FALSE,col.names=TRUE)

# Geoplot of NH census block groups and cluster centroids
pdf("c:/CONGRESSIONAL_DISTRICTING/NH_Census_Block_Group_Centroids.pdf")
plot(a[,1],a[,2],col="black",pch=19,cex=.25,asp=1.5,xlab="LONGITUDE",ylab="LATITUDE",
main="NEW HAMPSHIRE CENSUS BLOCK GROUPS & CLUSTER CENTROIDS")
points(MEAN_LONGITUDE,MEAN_LATITUDE,col="red",pch=19,cex=.75)
dev.off()

# Geoplot of NH census block group clusters
pdf("c:/CONGRESSIONAL_DISTRICTING/NH_Census_Block_Group_Clusters.pdf")
plot(a[,1],a[,2],asp=1.5,cex=.25,col=rainbow(k)[c],pch=19,xlab="LONGITUDE",ylab="LATITUDE",
main="NEW HAMPSHIRE CENSUS BLOCK GROUP CLUSTERS")
dev.off()
date() # Time
